...
- Restore baseline help examples and exit codes, and validate CLI route values independently of TOML field order; supersedes the unpublished 2.138.0 and 2.138.1 attempts.
...
- Made manifest-lock acceptance transactional with reviewable hash plans, entry-scoped
  compare-and-swap checks, bounded live-owner waiting, replay protection, and atomic lock
  replacement. Independent command fragments now merge into the latest shared lock instead of
  invalidating one another's plans, while edits to the same fragment still fail closed.
