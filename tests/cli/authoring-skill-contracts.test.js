...
test('completion evidence gate explicitly hands off useful follow-ups to next action menu', () => {
	...
	assert.match(
		i18n,
		/\[documents\."docs\.agent-workflow"\][\s\S]*?revision = 28/u,
	);
	assert.match(
		i18n,
...
test('design implementation handoff separates public specs from private agent state', () => {
	...
	assert.match(i18n, /\[documents\."skills\.index"\][\s\S]*?revision = 211/u);
...
test('Go code change skill gates runtime, concurrency, JSON, HTTP, and toolchain traps by Go version', () => {
	...
	assert.match(skillIndex, /HTTP clients or servers, graceful shutdown, reverse proxies, JSON encoding/u);
	...
	assert.match(i18n, /\[documents\."skills\.index"\][\s\S]*?revision = 211/u);
	assert.match(i18n, /\[documents\."skill\.go-code-change"\][\s\S]*?revision = 5/u);
...
test('small service platform architecture review keeps product factories registry-led and template-synced', () => {
	...
	assert.match(routes, /\[routes\."small-service-platform-architecture-review"\]\r?\ncategory = "architecture_patterns"\r?\nroute_type = "primary"\r?\npriority = 84/u);
...
test('complex decision analysis is narrow, falsifiable, and handoff-only before implementation', () => {
	...
	assert.match(
		routes,
		/applies_to_reasons = \["unknown_change", "product_change", "cross_cutting_code_change"\]/u,
	);
