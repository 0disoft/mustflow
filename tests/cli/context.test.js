...
test('prints all prompt-cache layers when requested', () => {
	...
	try {
		...
		assert.ok(context.task_context.route_read_plan.route_sources.includes('.mustflow/skills/catalog.v2.json'));
...
test('prints all prompt-cache audit layers without requiring an explicit profile', () => {
	...
	try {
		...
		if (context.cache_audit.layers[1].budget_status === 'over_budget') {
			assert.ok(
				context.cache_audit.layers[1].issues.some((issue) =>
					/task context measured sources exceed max_task_context_kb/u.test(issue)
				),
			);
		}
