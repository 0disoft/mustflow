...
test('HTTP delivery streaming skill keeps compression and browser transports explicit', () => {
	...
	assert.match(performanceSkill, /`Map` or `Set` O\(1\) folklore/u);
	assert.match(performanceSkill, /JavaScript engine evidence boundary/u);
	assert.match(performanceSkill, /V8-friendly object and array shapes/u);
	assert.match(performanceSkill, /Safari and Firefox need their own measurement/u);
	assert.match(performanceSkill, /repeated `Intl` formatter construction/u);
	assert.match(performanceSkill, /repeated Array built-in borrowing on `arguments` or `NodeList`/u);
	assert.match(performanceSkill, /Do not treat a Chrome or Node V8 profile as proof/u);
	assert.match(performanceSkill, /`shift\(\)` queues/u);
	assert.match(performanceSkill, /middle-`splice` deletes/u);
	assert.match(performanceSkill, /holey or mixed-kind arrays/u);
	assert.match(performanceSkill, /object pooling without allocation evidence/u);
	assert.match(performanceSkill, /Runtime and browser execution evidence/u);
	assert.match(performanceSkill, /initial JavaScript parse, compile, evaluate/u);
	assert.match(performanceSkill, /Main-thread contention evidence for browser paths/u);
	assert.match(performanceSkill, /microtask pressure, large JSON parsing, DOM mutation count/u);
	assert.match(performanceSkill, /event-loop utilization/u);
	assert.match(performanceSkill, /Frontend bundle execution/u);
	assert.match(performanceSkill, /Do not accept gzip-only savings/u);
	assert.match(performanceSkill, /Browser main-thread contention/u);
	assert.match(performanceSkill, /async callbacks, microtasks, JSON parsing, DOM insertion/u);
	assert.match(performanceSkill, /same interaction budget/u);
	assert.match(performanceSkill, /Node\.js runtime pressure/u);
	assert.match(performanceSkill, /sync API use/u);
	assert.match(performanceSkill, /REDOS/u);
	assert.match(performanceSkill, /runtime changes to `UV_THREADPOOL_SIZE`/u);
	assert.match(performanceSkill, /gzip-only budgets/u);
	assert.match(performanceSkill, /analyzer-only claims without route Coverage/u);
	assert.match(performanceSkill, /preload or prefetch priority inflation/u);
