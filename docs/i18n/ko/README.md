...
mustflow는 LLM 코딩 에이전트가 저장소에 진입했을 때 추측 없이 읽고 실행하며 검증할 수 있도록 지원하는 에이전트용 워크플로우 CLI입니다.

핵심 개념은 단순합니다. 사용자 프로젝트 루트에 `AGENTS.md`를 두고, 세부 워크플로우는 `.mustflow/` 폴더 아래에 모읍니다. 에이전트는 `AGENTS.md`에서 시작해 명령 규약, 작업 스킬, 프로젝트 맥락, 검증 절차를 순차적으로 확인합니다.
