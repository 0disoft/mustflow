...
- `mf dashboard`는 `.mustflow/config/preferences.toml`의 안전한 설정을 확인하고
  ...
  한국어, 중국어, 스페인어, 프랑스어, 힌디어 중에서 바꿀 수 있습니다.
...
| `mf dashboard` | 안전한 mustflow 설정을 위한 로컬 대시보드를 시작하고 기본 브라우저에서 엽니다. 화면 언어를 여섯 가지 언어 중에서 바꿀 수 있습니다. |
...
- `--set`: 설치 중 허용된 설정을 바꿉니다. 지원하는 키는
  ...
  `git.commit_message.language`, `reporting.commit_suggestion.enabled`,
  `language.memory.summary`입니다.
