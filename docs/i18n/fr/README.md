...
pnpm et Bun peuvent utiliser le même paquet npm. Ici, Bun est une option d’installation/exécution, pas une dépendance supplémentaire de mustflow.
...
Pour une installation locale au projet, utilisez `npx mf`, `pnpm exec mf` ou `bunx mf`. Pour lancer `mf` directement dans le shell, installez mustflow globalement.
...
Si le shell affiche encore `mf: command not found`, mustflow n'est pas installé globalement pour ce shell, ou le dossier global des exécutables du gestionnaire de paquets n'est pas dans `PATH`. Avec Bun, vérifiez que le dossier global des exécutables de Bun, généralement `~/.bun/bin`, est dans `PATH`.
...
| `mf api serve --stdio` | Sert les mêmes rapports API en lecture seule sous forme de réponses JSON délimitées par ligne sur stdin/stdout. |
| `mf api verification-plan --changed --json` | Affiche un verification plan stable et en lecture seule pour les fichiers modifiés sans exécuter de commandes. |
| `mf api latest-evidence --json` | Affiche les dernières preuves bounded de run ou verify sans sortie de commande brute. |
| `mf api diff-risk --changed --json` | Affiche un résumé compact du risque, de la vérification et des signaux residual correction en lecture seule des fichiers modifiés. |
| `mf api health --json` | Affiche un rapport compact de santé du workspace pour les gates rapides d’agent. |
| `mf evidence --changed` | Résume les exigences de vérification des changements, les preuves récentes, les reçus et les écarts restants sans exécuter de commandes. |
| `mf workspace status` | Inspecte les racines workspace configurées et l'état des contrats imbriqués sans accorder d'autorité de commande du parent vers l'enfant. |
| `mf workspace command-catalog` | Affiche la disponibilité des intents par dépôt et les entrées `mf run` sûres sans chaînes de commande brutes. |
| `mf workspace command-fragments` | Suggère des fragments `.mustflow/config/commands/*.toml` par dépôt pour l'orchestration repo-farm sans écrire de fichiers ni accorder d'autorité de commande du parent vers l'enfant. |
| `mf workspace verify --changed --plan-only` | Affiche les plans de vérification des changements par dépôt sans exécuter de commandes. |
| `mf api locks --json` | Affiche les verrous `mf run` actifs pour coordonner plusieurs sessions. |
...
| `mf version`                | Affiche la version installée du paquet mustflow.                                        |
| `mf version --check`        | Compare la version installée avec la dernière version publiée sur npm et affiche une commande de mise à jour. |
...
- `--set` : définit une préférence autorisée pendant l’installation. Les clés prises en charge sont `git.auto_stage`, `git.auto_commit`, `git.auto_push=false`, `git.commit_message.*`, `reporting.commit_suggestion.enabled`, `language.memory.summary`, `release.versioning.*`, `verification.selection.*` et `testing.authoring.*`.
  ...
  - `git.commit_message.gitmoji.map=conventional_default` déclare la correspondance des types conventional vers les préfixes Gitmoji.
  - `git.commit_message.body.template=summary_validation` demande un corps qui résume le changement et les preuves de validation ; `git.commit_message.body.require_validation_line` accepte `true` ou `false`.
