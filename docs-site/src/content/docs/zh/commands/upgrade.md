...
它不会安装包。若有新版本，会先打印对应包管理器的更新命令并停止。只有 `Blocked local changes` 与 `Manual review` 都为 `0` 时，才会写入当前模板 manifest 中的 `update`、`create` 项以及仍由锁文件拥有的退役 `remove` 项；替换或删除前会备份到 `.mustflow/backups/<timestamp>/`。
