...
`mf update --dry-run --diff` 会使用同一个计划，并在不写入文件的前提下添加受限的 unified diff 预览。
...
Diff 预览会比较受影响文件的当前目标内容与打包模板内容。它可用于将创建、将更新、因本地变更被阻止或需要人工复核的项目。

预览输出有大小限制。达到限制时，mustflow 会打印明确的截断标记。生成状态、缓存、备份路径或看起来敏感的文件名会被省略，而不是打印内容。
...
- `items[].diff_preview` (`object`)：仅在使用 `--diff` 且该项受影响时出现。它包含受限的 unified diff 预览、可用性、截断状态、行数限制，以及不适合打印时的省略原因。
