...
`.mustflow/config/preferences.toml` 中的 `[release.versioning]` 控制版本影响报告偏好。

当代码、模板、架构、CLI 行为、包元数据、用户可见文档、安装输出或测试发生变化时，agent 应检查该变更是否看起来需要更新包版本或模板版本。

默认情况下，如果证据明确，mustflow 可以建议 patch、minor 或 major 升级，但除非用户明确要求版本升级或 release 准备，否则不得编辑版本文件。
...
当版本变更获得批准时，应根据 `sync_*` 偏好让包元数据、模板清单版本、文档示例和测试保持同步。
