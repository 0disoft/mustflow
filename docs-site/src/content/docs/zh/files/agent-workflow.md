...
- `Context compaction policy`: 说明近期原始上下文、中层摘要和长期摘要的边界与权威顺序。
...
agent 只能运行同时满足 `status = "configured"`、`lifecycle = "oneshot"`、`run_policy = "agent_allowed"` 和 `stdin = "closed"` 的命令意图。如果意图缺失，或状态为 `unknown`、`not_applicable`、`manual_only`、`disabled`，agent 不得推断替代命令，必须报告跳过原因。
...
agent 应在首次编辑前、命令执行前、上下文压缩后、编辑 `AGENTS.md` 或 `.mustflow/**` 后、切换根目录后，以及最终报告前刷新 mustflow 指令。
