...
- `Skill activation`: 定义 agent 何时选择并读取任务相关 skill 流程。
...
- `Harness 与 runtime 表面`: 说明仓库本地工作流文件如何扩展到可选 runtime、协调、工作项、适配器和 harness 表面。
...
- `版本影响策略`: 检查变更是否应建议版本升级，同时默认禁止自动修改版本文件。
...
`.mustflow/skills/<name>/SKILL.md`，并在当前命令契约内遵循该流程。

在任务开始和首次编辑前，agent 会把用户请求和预期变更文件与
`.mustflow/skills/INDEX.md` 进行比较。如果一个或多个场景匹配，先读取对应的
`SKILL.md` 再编辑该范围。如果之后出现新的证据，例如命令失败、测试契约变化或文档变化，
agent 会暂停并激活新相关的 skill。

skill 永远不会授权原始 shell 命令、长时间运行进程或任务范围外写入。它们也不会覆盖
用户、宿主、仓库或安全规则。
...
## Harness 与 runtime 表面

mustflow 从仓库本地工作流文件和命令执行边界开始。只要这些表面是明确、有边界、可审查的，它也可以扩展到可选 runtime、协调、工作项、适配器和 harness 表面。

- Brain 表面：`AGENTS.md`、`agent-workflow.md` 和 skill 文档。
- Hands 表面：`commands.toml`、`mf run` 和有限命令生命周期。
- Session 表面：有边界的运行 receipt、带来源摘要、紧凑交接记录和未来工作项记录。
- Judge 表面：原始目标、验收标准、已变更文件、命令合同和 receipt。
- Runtime 表面：workers、personas、fleets、service processes 和自治循环需要声明生命周期、保留、隔离、审批和验证规则。
...
`.mustflow/config/preferences.toml` 中的 `[release.versioning]` 控制版本影响偏好，也包括 agent 在定位仓库实际版本来源后是否可以编辑版本文件。

当代码、模板、架构、CLI 行为、包元数据、用户可见文档、安装输出或测试发生变化时，agent 应检查该变更是否看起来需要更新包版本或模板版本。

默认情况下，当证据明确、版本来源已定位，且 `auto_bump = true`、`require_user_confirmation = false` 时，mustflow 可以直接应用 patch、minor 或 major 升级。用户直接指令、宿主安全规则或审批策略仍会阻止自动版本编辑。

在建议或应用版本变更前，agent 必须找到仓库实际的版本来源，而不是假设它就是 `package.json`。常见候选包括 `package.json`、`pyproject.toml`、`Cargo.toml`、与 release tag 一起查看的 `go.mod`、`pom.xml`、`build.gradle`、`*.csproj`、`*.gemspec`、`composer.json`、`pubspec.yaml`、`Package.swift`、`Chart.yaml`、应用 manifest、release notes 和 mustflow 模板 manifest。

版本发生变更时，应根据 `sync_*` 偏好让包元数据、模板清单版本、文档示例和测试保持同步。
