...
这些值是偏好设置，不是发布权限。`impact_check = true` 要求 agent 报告当前差异是否看起来需要包版本或模板版本变更。`suggest_bump = true` 允许在证据清楚时建议 patch、minor 或 major。

`auto_bump = false` 会保持包和模板版本文件不变，除非用户明确要求提升版本或准备发布。`require_user_confirmation = true` 表示 agent 不得在普通代码改动中静默修改版本。

当版本变更已获批准时，`sync_template_version`、`sync_docs_examples` 和 `sync_tests` 要求 agent 在同一改动中同步包元数据、模板 manifest、文档示例和测试。
