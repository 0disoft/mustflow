...
Quand `mf verify` exécute réellement des commandes, il utilise le même modèle de planification que la sortie plan-only et adopte un parallélisme sûr de 4 par défaut, limité par le CPU local et un plafond de 8. Seules les entrées d'un même lot avec effets explicites et sans conflit peuvent s'exécuter ensemble ; les commandes sans métadonnées d'effet suffisantes restent conservatrices. Utilisez `--parallel 1` pour une exécution série. Les reçus restent écrits dans l'ordre planifié, et la sortie verify, le manifeste, le pointeur latest et les reçus partagent le même `verification_plan_id`.

`--profile edit|commit|release` adapte la vérification à l'étape du travail. `edit`, valeur par défaut, choisit les contrôles suffisants les plus petits dans un budget déclaré de 15 secondes ; `commit` élargit ce budget à 60 secondes ; `release` conserve tous les contrôles applicables. Les raisons de sécurité, confidentialité, données, migration, métadonnées de paquet, empaquetage et publication conservent toujours la vérification complète. Un reçu réussi antérieur n'est réutilisé que si le plan et le hash de l'état courant correspondent, si le profil n'est pas `release` et si le risque est inférieur à high.
