...
Cuando `mf verify` ejecuta comandos, usa el mismo modelo de planificación que la salida plan-only y adopta un paralelismo seguro predeterminado de 4, limitado por la CPU local y un máximo de 8. Solo las entradas del mismo lote con efectos explícitos y sin conflictos pueden ejecutarse a la vez; los comandos sin metadatos de efectos suficientes se mantienen conservadores. Usa `--parallel 1` para ejecución en serie. Los recibos se escriben en el orden planificado, y la salida de verify, el manifiesto, el puntero latest y los recibos comparten el mismo `verification_plan_id`.

`--profile edit|commit|release` ajusta la verificación a la etapa de trabajo. `edit`, el valor predeterminado, elige las comprobaciones suficientes más pequeñas dentro de un presupuesto declarado de 15 segundos; `commit` amplía el presupuesto a 60 segundos; `release` conserva todas las comprobaciones aplicables. Las razones de seguridad, privacidad, datos, migración, metadatos de paquete, empaquetado y publicación siempre mantienen la verificación completa. Un recibo exitoso anterior solo se reutiliza si coinciden el plan y el hash del estado actual, el perfil no es `release` y el riesgo es inferior a high.
