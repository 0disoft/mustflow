...
When `mf dashboard` saves `.mustflow/config/preferences.toml`, it refreshes that tracked file as `last_action = "customized"` if `manifest.lock.toml` exists. This records the accepted local preference edit without treating the whole lock file as a live snapshot.
...
The lock file therefore records the installation history rather than acting as a live snapshot of the current state.
...
Dedicated baseline-acceptance commands use entry-scoped plans. They merge reviewed entries into
the latest lock under a short-lived owner record, so unrelated command fragments do not invalidate
one another's plans. The same entry cannot be accepted concurrently, and a plan cannot be replayed.
