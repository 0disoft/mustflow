...
`REPO_FLOW.md` belongs to `document_roots.generated`. It is generated by `mf flow` as a design-flow navigation aid and does not replace `AGENTS.md`, `.mustflow/config/commands.toml`, or current source files.
...
`REPO_FLOW.md` follows the same generated-file hygiene: stable frontmatter, source fingerprint,
and no timestamps, branch names, remote URLs, local absolute paths, or recent-change logs.
