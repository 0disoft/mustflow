...
- `task_context.repo_map_read_plan` (`object`): Static read plan for `repo_map_navigation`. It prefers local-index source anchors and selected `REPO_MAP.md` anchor spans before falling back to the full `REPO_MAP.md`, which is marked avoid-by-default.
