...
When `mf verify` actually runs commands, it uses the same schedule model as plan-only output and defaults to safe parallelism of 4, capped by local CPU and the hard limit of 8. Before each selected intent starts, it acquires one shared host and repository CPU token and one memory token; intents with declared writes also acquire a disk token. Concurrent shells, agents, and worktrees therefore cannot each claim the full machine independently, while idle or serial verification sessions do not reserve unused capacity. Worktrees linked through Git `commondir` share one repository budget. Only entries from the same explicit-effect, non-conflicting schedule batch can run together; commands without sufficient effect metadata remain conservative. Use `--parallel 1` for serial execution. Receipts are written in schedule order, and the verify output, verify bundle manifest, latest pointer, and per-intent receipts share the same `verification_plan_id`.

`--profile edit|commit|release` prices verification by workflow stage. `edit` is the default and targets the smallest sufficient configured checks within a 15-second declared-cost budget. `commit` widens that budget to 60 seconds. `release` retains all applicable checks. Security, privacy, data, migration, packaging, and release reasons always retain full verification. Package metadata alone follows the selected profile; packaging or release changes still require their full applicable checks. A successful earlier receipt may be reused only when its verification plan and current-state hash match, the selected profile is not `release`, and risk is below high.
