...
`mf check` संरचनात्मक उल्लंघनों को blocking errors मानता है। कोई भी reported issue code `1` के साथ exit करता है।
...
- Strict errors अतिरिक्त document identity, routing, skill metadata, command boundary, repository map, retention, run receipt और context hygiene checks से आते हैं। ये केवल `--strict` enabled होने पर दिखते हैं।
- Non-blocking observations को `mf check` issues के रूप में emit नहीं किया जाता। जब automation को informational या warning-level health signals चाहिए, तब `mf doctor` diagnostics उपयोग करें।
...
- `issueDetails` (`object[]`): machine-readable समस्या विवरण। लागू होने पर `id` command boundary और संबंधित strict checks के लिए स्थिर identifier है, `severity` अभी हर blocking check issue के लिए `error` है, `mode` `base` या `strict` है, और `message` `issues` जैसा ही संदेश रखता है।
