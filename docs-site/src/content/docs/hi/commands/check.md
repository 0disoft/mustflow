...
- `.mustflow/skills/*/SKILL.md` फ़ाइलों में आवश्यक स्थिर सेक्शन पहचानकर्ता हैं।
...
- Source anchors structured code navigation hints ही रहने चाहिए। Strict mode malformed anchor declarations, duplicate anchor IDs, anchors में agent command या policy instructions, secret जैसे anchor text, generated या vendor paths में anchors, और unknown risk tags पर fail करता है।
- Source anchor quality signals, जैसे बहुत लंबा `purpose`, बहुत अधिक `search` terms, या एक file में बहुत अधिक anchors, warnings के रूप में emit होते हैं और check fail नहीं करते।
- authorization, personal data, payment, migration, data loss, secrets या security जैसे high-risk tags वाले source anchors lower warning thresholds उपयोग करते हैं, और `invariant` missing होने पर review warning देते हैं।
...
## Repository-scoped check

```sh
npx mf check --strict --repo projects/example
```

`--repo` delegated workspace mapping में घोषित एक repository चुनता है। Scoped check root defaults,
workspace mapping, selected command fragment और उनके manifest-lock entries validate करता है। Selected
scope में drift blocking error ही रहता है; unrelated manifest drift deferred warning के रूप में report
होता है। Scoped success whole-root success नहीं है। Global strict check केवल aggregate root changes या
explicit whole-root audit के लिए उपयोग करें।

## त्रुटि और चेतावनी वर्गीकरण

`mf check` संरचनात्मक उल्लंघनों को blocking errors मानता है। Blocking errors code `1` के साथ exit करते हैं; warnings अलग से report होती हैं और command fail नहीं करतीं।

- Base errors missing required files, parse failures, unsafe configuration values, command-contract violations, missing skill section identifiers, invalid context document identity और manifest-lock drift से आते हैं।
- Strict errors अतिरिक्त document identity, routing, skill metadata, source anchors, command boundary, repository map, retention, run receipt और context hygiene checks से आते हैं। ये केवल `--strict` enabled होने पर दिखते हैं।
- Non-blocking observations JSON output में `warnings` या human-readable output में warning lines के रूप में दिख सकती हैं। जब automation को व्यापक informational health signals चाहिए, तब `mf doctor` diagnostics उपयोग करें।
...
## मानक skill सेक्शन पहचानकर्ता

skill दस्तावेज़ों में स्थानीयकृत headings से पहले ये स्थिर सेक्शन पहचानकर्ता शामिल होने चाहिए।
...
विफल होने पर यह missing files या section identifiers को standard error पर प्रिंट करता है और code `1` के साथ बाहर निकलता है।
...
- `scope` (`object | null`): चुना हुआ delegated repository scope। Scoped result में `kind`, `repository`, और `contract` होते हैं; global result में `null` होता है।
- `issueCount` (`number`): मिली हुई समस्याओं की संख्या।
- `issues` (`string[]`): व्यक्ति-पठनीय समस्या संदेश।
- `warningCount` (`number`): मिली हुई non-blocking warnings की संख्या।
- `warnings` (`string[]`): व्यक्ति-पठनीय warning messages।
- `issueDetails` (`object[]`): machine-readable समस्या और warning विवरण। लागू होने पर `id` command boundary और संबंधित strict checks के लिए स्थिर identifier है, `severity` `error` या `warning` है, `mode` `base` या `strict` है, और `message` `issues` या `warnings` की संबंधित entry रखता है।

जब blocking समस्याएं मिलती हैं, JSON रूप भी code `1` के साथ बाहर निकलता है। केवल warnings होने पर exit code `0` रहता है।
