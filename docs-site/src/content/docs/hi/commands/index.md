...
`enabled_by_default = true` से `mf index` बिना `--source` के भी source anchors शामिल करता है। Include और exclude patterns सिर्फ scan bound करते हैं। Generated, dependency, और vendor paths local source index से बाहर ही रहते हैं, भले वे include pattern से match करें। Source files default रूप से 262144 bytes तक capped हैं; `max_file_bytes` यह ceiling कम कर सकता है, बढ़ा नहीं सकता।
...
- `content_mode` (`string`): stored content policy। अभी `metadata_and_snippets`।
- `store_full_content` (`boolean`): local index read model के लिए हमेशा `false`।
- `max_snippet_bytes_per_document` (`number`): हर document के लिए stored snippet का maximum byte size।
- `excluded_raw_data_kinds` (`string[]`): raw data categories जिन्हें SQLite index store नहीं कर सकता।
