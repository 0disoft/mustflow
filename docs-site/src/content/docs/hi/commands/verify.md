...
`mf verify --from-classification <path>` mustflow root के अंदर मौजूद `mf classify` JSON report से verification reasons पढ़ता है। `--from-plan` deprecated compatibility alias के रूप में available रहता है, लेकिन वही classification report पढ़ता है; यह `mf verify --plan-only --json` output नहीं पढ़ता।
...
जब `mf verify` commands चलाता है, तो वह plan-only output वाला same schedule model use करता है और default safe parallelism 4 रखता है, जिसे local CPU और hard limit 8 cap करते हैं। केवल same explicit-effect, non-conflicting schedule batch की entries साथ चल सकती हैं; पर्याप्त effect metadata के बिना commands conservative रहते हैं। Serial execution के लिए `--parallel 1` use करें। Receipts schedule order में लिखे जाते हैं, और verify output, bundle manifest, latest pointer, तथा per-intent receipts same `verification_plan_id` share करते हैं।

`--profile edit|commit|release` workflow stage के अनुसार verification cost तय करता है। Default `edit` declared 15-second budget में smallest sufficient checks चुनता है, `commit` budget को 60 seconds तक बढ़ाता है, और `release` सभी applicable checks रखता है। Security, privacy, data, migration, package metadata, packaging, और release reasons हमेशा full verification रखते हैं। पुराना successful receipt तभी reuse होता है जब plan और current-state hash match हों, profile `release` न हो, और risk high से कम हो।
...
- `run_dir` (`string`): unique verification bundle directory, जिसमें manifest और per-intent receipts होते हैं।
