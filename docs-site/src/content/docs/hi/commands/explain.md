...
- `topic` (`string`): `authority`, `command`, `retention`, `skill`, या `skills`.
- `mustflow_root` (`string`): मौजूदा mustflow root.
- `decision` (`object`): resolved decision, reason, effective action, source files, verification status, और topic-specific details.
