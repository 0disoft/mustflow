...
Fresh local index होने पर verify explanations matching candidates के लिए read-only command-effect graph status भी शामिल करते हैं। Index missing या stale हो तो rebuild hint दिखता है और command selection नहीं बदलता।
...
- `Verification explanation`: `verify` topic के लिए reasons, matching `required_after` intents, runnable candidates, skipped candidates, gaps, और local-index command-effect status.
