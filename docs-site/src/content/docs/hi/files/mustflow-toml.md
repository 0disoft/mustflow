...
- `compaction`: recent raw context, mid summaries, long summaries, और raw retention अलग रखने की policy।
...
`compaction` declare करता है कि long session context को recent raw input, mid-level summaries, और long-term summaries में कैसे अलग कर सकती है। यह default रूप से disabled है और इसका अर्थ यह नहीं कि mustflow full chat transcripts collect करता है।
...
`budget` iterations, elapsed time, command runs, output volume, और repeated failures सीमित करके unbounded loops रोकता है। Limit पहुंचने पर agents रुकते हैं और report या hand off करते हैं।
