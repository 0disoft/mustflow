...
- `Skill activation`: agents को task-specific skill procedures कब चुननी और पढ़नी हैं, परिभाषित करता है।
...
- `Version impact policy`: जांचता है कि repository preferences के अनुसार change के लिए version bump apply या suggest करना चाहिए या नहीं।
...
Agents केवल ऐसे command intents चला सकते हैं जो हर execution gate पूरा करें: `status = "configured"`, `lifecycle = "oneshot"`, `run_policy = "agent_allowed"`, `stdin = "closed"`, positive `timeout_seconds`, `argv` या `mode = "shell"` plus `cmd` से declared command, और current mustflow root के अंदर `cwd`। यदि कोई intent missing, `unknown`, `not_applicable`, `manual_only`, या `disabled` है, तो agents replacement command infer नहीं कर सकते और skipped reason report करना होगा।

नई configurations में `manual_only` को intent `status` के रूप में उपयोग करें। पुराने configs के लिए `run_policy = "manual_only"` स्वीकार हो सकता है, लेकिन नई templates इसे generate नहीं करतीं।
...
Verification selection risk-based है। Agents को related tests, fast checks, build, या docs-specific
validation prefer करनी चाहिए जब वे intents बदले हुए surface को cover करते हों, और narrow intent
missing हो तो slow full-suite tests को चुपचाप default बनाने के बजाय gap report करना चाहिए।
...
Host shell commands चला सकता है, लेकिन project commands को सीधे चलाने से वे अपने-आप mustflow verification नहीं बनते।
यदि कोई command `mf run` को bypass करती है, तो उसकी output को lower-confidence context मानें, जब तक user ने explicitly manual override approve न किया हो और final report यह न बताए कि कोई mustflow run receipt नहीं बना।
...
`.mustflow/skills/<name>/SKILL.md` पढ़ना और current command contract के भीतर उस procedure
का पालन करना।

Task start और first edit से पहले agents user request और expected changed files को
`.mustflow/skills/INDEX.md` से compare करते हैं। यदि एक या अधिक scenarios match हों, तो उस
scope को edit करने से पहले matching `SKILL.md` files पढ़ी जाती हैं। यदि बाद में command failure,
test contract change, या documentation change जैसी नई evidence आए, तो agents रुककर newly
relevant skill activate करते हैं।

Skills raw shell commands, long-running processes, या task scope के बाहर writes authorize नहीं
करतीं। वे user, host, repository, या safety rules को override भी नहीं करतीं।
...
सभी instructions को एक priority list में न मिलाएं। Conflict को rule के प्रकार से resolve करें:

- User goal: current direct user instructions task define करती हैं, जब तक वे unsafe न हों।
- Host safety: host approval, sandbox, checkpoint, और shell execution gates binding रहते हैं।
- Repository work rules: nearest `AGENTS.md` और `.mustflow/config/*.toml` repository contract define करते हैं।
- Command execution: `.mustflow/config/commands.toml` project command contract define करता है।
- Verification evidence: `mf run` receipts और current files direct host shell output से stronger evidence हैं।
- Context and preferences: `.mustflow/context/*`, `preferences.toml`, और generated maps lower-authority defaults हैं।
- Session and cache state: host summaries, `.mustflow/cache/**`, और `.mustflow/state/**` कभी भी current files या current user instructions override नहीं करते।

Allowed action sets intersection से narrow होते हैं। Denied actions, approval requirements, privacy rules, और destructive-command rules accumulate होते हैं। Effective rule unclear हो तो guess करने के बजाय रुकें और conflict report करें।
...
Default रूप से, evidence साफ हो, version source मिल गया हो, और `auto_bump = true` के साथ `require_user_confirmation = false` हो, तो mustflow patch, minor, या major bump सीधे apply कर सकता है। Direct user instruction, host safety rule, या approval policy automatic version edits को अब भी block करती है।
