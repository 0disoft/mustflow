...
ये values version-impact reporting और version-file edits को guide करने वाली preferences हैं। `impact_check = true` agent से यह report करवाता है कि diff package या template version change मांगता है या नहीं। `suggest_bump = true` clear evidence होने पर patch, minor, या major suggestion की अनुमति देता है।

`auto_bump = true` agent को version source मिलने के बाद सही package या template version bump apply करने देता है, जब तक direct user instruction, host safety rule, या approval policy उसे block न करे। `auto_bump = false` package और template version files को untouched रखता है, जब तक user version bump या release-preparation task न मांगे। `require_user_confirmation = true` का मतलब है कि agent versions edit करने से पहले पूछे; `false` automatic bump enabled होने पर वह extra confirmation step हटाता है।

Version change के समय `sync_template_version`, `sync_docs_examples`, और `sync_tests` agent को package metadata, template manifests, documentation examples, और tests को उसी change में aligned रखने को कहते हैं।
...
`[testing.authoring]` guide करता है कि agent नए tests बनाए या पहले existing coverage के साथ काम करे। यह `[verification.selection]` से अलग है: verification selection configured checks चुनने में मदद करता है, जबकि test authoring बताता है कि नए test files या cases कितनी आसानी से जोड़े जाएं।

`new_test_policy = "evidence_required"` default है। इसका मतलब है कि नए tests behavior-contract evidence पर आधारित होने चाहिए, जैसे बदला public behavior, regression risk, configuration या schema impact, या security/data-path risk।

`new_test_policy = "manual_approval"` agent से कहता है कि user ने सीधे tests नहीं मांगे हों तो नए tests जोड़ने से पहले confirm करे। `new_test_policy = "broad"` important behavior स्पष्ट करने पर proactive test authoring की अनुमति देता है।

`prefer_existing_tests = true` agent से कहता है कि नए files या cases बनाने से पहले nearby existing tests update करे। `require_new_test_rationale = true` final report में हर नए test की जरूरत explain करने को कहता है।

ये preferences required verification skip करने, valid tests हटाने, या assertions loosen करने का कारण नहीं हैं।
...
- `[testing.authoring]` allowed new-test policy और boolean authoring flags उपयोग करता है।
