...
`mf check`는 구조 위반을 차단 오류로 처리합니다. 보고된 문제가 하나라도 있으면 종료 코드 `1`로 끝납니다.

- 기본 오류는 필수 파일 누락, 파싱 실패, 안전하지 않은 설정값, 명령 계약 위반, 스킬 필수 섹션 식별자 누락, 컨텍스트 문서 식별 오류, 잠금 파일 기준 이탈에서 나옵니다.
- 엄격 오류는 문서 식별 정보, 라우팅, 스킬 메타데이터, 명령 경계, 저장소 지도, 보존 정책, 실행 기록, 컨텍스트 위생 상태를 추가로 검사할 때 나옵니다. 이 항목은 `--strict`를 켰을 때만 표시됩니다.
- 차단하지 않는 관찰 결과는 `mf check`의 문제 목록에 넣지 않습니다. 자동화에서 정보 또는 경고 수준의 상태 신호가 필요하면 `mf doctor` 진단 정보를 사용합니다.
...
- `issueDetails` (`object[]`): 기계가 읽기 좋은 문제 상세 목록입니다. `id`는 명령 경계 및 관련 엄격 검사에 해당할 때 안정적인 식별자를 담고, `severity`는 현재 모든 차단 검사 문제에서 `error`이며, `mode`는 `base` 또는 `strict`입니다. `message`는 `issues`와 같은 설명을 담습니다.
