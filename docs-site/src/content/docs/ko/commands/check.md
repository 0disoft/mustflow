...
`--repo`는 위임된 작업공간 매핑에 선언된 저장소 하나를 선택합니다. 범위 지정 검사는 루트 기본값,
작업공간 매핑, 선택된 명령 조각과 해당 manifest-lock 항목을 검사합니다. 선택 범위의 변경은 계속
차단 오류이며, 관련 없는 manifest 항목의 변경은 보류 경고로만 보고합니다. 범위 지정 성공은 루트
전체 성공이 아닙니다. 전역 엄격 검사는 루트 전체 변경이나 명시적으로 요청한 전체 감사에만 사용하세요.
...
- `scope` (`object | null`): 선택된 위임 저장소 범위입니다. 범위 지정 결과는 `kind`, `repository`, `contract`를 포함하며, 전역 결과는 `null`입니다.
