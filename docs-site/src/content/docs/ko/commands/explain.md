...
최신 로컬 색인이 있으면 맞은 후보의 읽기 전용 명령 효과 그래프 상태도 포함합니다. 색인이 없거나 오래되었으면 다시 색인하라는 안내만 표시하며 명령 선택은 바꾸지 않습니다.
...
- `검증 설명`: `verify` 주제를 사용할 때의 검증 이유, 맞은 `required_after` 명령 의도, 실행 가능한 후보, 건너뛴 후보, 빈틈, 로컬 색인의 명령 효과 상태입니다.
