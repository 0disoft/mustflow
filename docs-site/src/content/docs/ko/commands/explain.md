...
`mf explain --why-blocked <intent>`는 같은 명령 판단을 `mf run` 사전 계획 관점으로 좁혀 보여줍니다. 명령을 실행하지 않고 차단 이유 코드, 제한된 실행 계획 메타데이터, 제안 명령 계약 조각을 출력합니다.
...
`mf explain skill <skill_id>`는 `.mustflow/skills/INDEX.md`의 특정 스킬 라우트 하나를 설명합니다. 트리거, 필요한 입력, 수정 범위, 위험, 검증 의도, 기대 출력 형식, 선택 근거를 함께 보여줍니다. 대상은 스킬 폴더 이름, 전체 `metadata.skill_id`, `mustflow_doc`, 또는 스킬 경로로 지정할 수 있습니다.
...
`mf explain why <target>`은 다른 `explain` 주제가 쓰는 기존 판단 모델을 통해 결정 이유를 설명합니다. 새 선택기를 만들지 않는 읽기 전용 래퍼입니다. 대상은 `command <intent>`, `intent <intent>`, `verify --reason <event>`, `verify --from-plan <path>`, `skill <skill_id>`, `skills`, `surface [path]`, `latest-failure`를 지원합니다.
`mf explain why latest-failure`는 `.mustflow/state/runs/latest.json`에서 제한된 메타데이터만 읽습니다. 상태, 명령 의도, 종료 코드, 오류 종류, 걸린 시간, 짧은 요약만 보여주며 표준 출력이나 표준 오류 꼬리는 출력하지 않습니다.
...
- `차단된 실행 계획`: `--why-blocked`를 사용할 때의 `mf run` 사전 계획 상태, 이유 코드, 세부 정보, 설정된 cwd, 타임아웃, 모드, 쓰기 경로, 제안 조각입니다.
...
- `스킬 선택 근거`: `skill` 주제를 사용할 때 맞은 스킬 식별자, 선언된 필수 입력, 정적으로 확인된 누락 입력, 후보 보조 스킬, 맞지 않은 경로, 남은 빈틈 메모를 보여줍니다.
...
- `최근 실행 실패`: `mf explain why latest-failure`를 사용할 때 최근 실행의 상태, 명령 의도, 종료 코드, 오류 종류, 걸린 시간, 요약을 제한된 범위로 보여줍니다.
...
- `topic` (`string`): `anchor`, `asset-optimization`, `authority`, `command`, `retention`, `skill`, `skills`, `surface`, `verify`, `why` 중 하나입니다.
- `mustflow_root` (`string`): 현재 mustflow 루트입니다.
- `decision` (`object`): 해석된 결정, 이유, 적용할 조치, 근거 파일, 검증 인정 여부, 주제별 세부 정보입니다. `authority` 주제에서는 `boundary.role`, `boundary.canDefine`, `boundary.cannotDefine`도 포함합니다. `command` 주제에서는 선언된 명령 의도에 대해 `decision.effectGraph`가 로컬 색인의 명령 효과 그래프 상태, 쓰기 잠금, 충돌, 오래된 경로, 다시 색인 안내를 담습니다. `--why-blocked`에서는 `decision.blockedRunPlan`이 `mf run` 사전 계획의 이유 코드, 세부 정보, 메타데이터, 제안 조각을 담습니다. `skill` 주제에서는 `decision.selectionEvidence`가 `matchedBy`, `requiredInputs`, `missingInputs`, `candidateAdjuncts`, `unmatchedPaths`, `gapNotes` 같은 결정적 라우팅 근거를 담습니다. `verify` 주제에서는 `decision.verification`이 선택된 검증 이유, 맞은 후보, 건너뜀 이유, 빈틈, `decisionGraph`, 로컬 색인의 명령 효과 상태를 담습니다. `surface` 주제에서는 사용할 수 있을 때 `decision.readModel`이 읽기 전용 로컬 색인의 경로-표면 상태와 맞은 규칙 메타데이터를 담습니다. `why latest-failure`에서는 `decision.latestFailure`가 로그 꼬리 없이 제한된 최근 실행 메타데이터를 담습니다.
