...
`mf explain surface [path]`는 저장소 상대 경로가 변경 분류에서 쓰는 공개 표면 계약에 어떻게 대응되는지 설명합니다. 검증을 실행하지 않고 표면 종류, 검증 이유, 영향받는 계약, 갱신 정책, 어긋남 검사를 보여줍니다.
...
- `topic` (`string`): `anchor`, `asset-optimization`, `authority`, `command`, `retention`, `skill`, `skills`, `surface` 중 하나입니다.
