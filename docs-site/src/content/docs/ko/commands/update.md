...
`mf update --dry-run`은 `manifest.lock.toml`을 기준으로 현재 파일이 설치 당시와 달라졌는지 확인하고, 템플릿 갱신이 가능한지 계획만 출력합니다.
`mf update --apply`는 차단된 로컬 변경이나 수동 검토 항목이 없을 때만 `update`와 `create` 항목을 실제로 적용합니다.
자동화나 에이전트가 계획을 파싱해야 한다면 `--json`을 함께 사용합니다.
...
mustflow 파일은 에이전트 작업 규칙과 절차를 담습니다. 사용자가 직접 고친 파일을 자동으로 덮어쓰면 저장소별 규칙을 잃을 수 있습니다.

그래서 갱신 명령은 먼저 다음을 구분해야 합니다.

- 현재 파일이 설치 당시 해시와 같은가
- 현재 파일이 새 템플릿과 다른가
- 사용자가 수정한 로컬 변경이 있는가
- 자동 갱신보다 수동 검토가 필요한가
...
로컬 변경이 있으면 종료 코드 `1`로 끝납니다. 이 경우 사용자가 변경을 확인한 뒤 수동으로 정리해야 합니다.
...
새 템플릿 파일이 사용자 저장소에 이미 존재하지만 잠금 파일에 기록되어 있지 않고 내용도 다르면, mustflow는 그 파일을 로컬 변경으로 보고 덮어쓰지 않습니다.
...
- `policy.never_overwrite_local_changes` (`boolean`): 로컬 변경 파일을 자동으로 덮어쓰지 않는다는 선언입니다.
...
- `summary.wouldUpdate` (`number`): 향후 갱신 명령이 바꿀 수 있는 파일 수입니다.
- `summary.wouldCreate` (`number`): 향후 갱신 명령이 만들 수 있는 파일 수입니다.
