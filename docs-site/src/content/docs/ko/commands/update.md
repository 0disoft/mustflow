...
`mf update --dry-run --diff`는 같은 계획을 기준으로 파일을 쓰지 않은 채 제한된 unified diff 미리보기를 덧붙입니다.
...
차이 미리보기는 영향을 받는 파일의 현재 내용과 번들 템플릿 내용을 비교합니다. 생성 예정, 갱신 예정, 로컬 변경으로 차단된 항목, 수동 검토 항목에 대해 표시될 수 있습니다.

미리보기 출력은 크기가 제한됩니다. 제한에 걸리면 mustflow가 명시적인 잘림 표시를 출력합니다. 생성된 상태, 캐시, 백업 경로, 민감 정보처럼 보이는 파일 이름은 내용을 출력하지 않고 생략 사유만 남깁니다.
...
- `items[].diff_preview` (`object`): `--diff`를 사용했고 영향받는 항목일 때만 포함됩니다. 제한된 unified diff 미리보기, 출력 가능 여부, 잘림 상태, 줄 제한, 안전하게 출력할 수 없어 생략한 이유를 담습니다.
