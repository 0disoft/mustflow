...
변경 block을 찾을 때 사용합니다. 또한 stable prefix 보존 여부, 앞에서부터 일치한 stable block 수,
첫 stable 무효화 사유를 함께 보고해 source 본문을 노출하지 않고 cache miss 원인을 추적할 수 있습니다.
...
- `prompt_bundle_diff` (`object | undefined`): `--cache-compare`를 사용할 때만 나옵니다. 현재 `prompt_bundle`과 baseline context JSON 보고서를 비교해 `unchanged`, `changed`, `baseline_missing`, `baseline_unreadable`, `baseline_invalid`, `baseline_without_prompt_bundle` 중 하나를 보고합니다.
...
- `prompt_bundle_diff.stable_diff` (`object`): 현재 prompt bundle과 baseline prompt bundle의 stable prefix 비교 요약입니다.
- `prompt_bundle_diff.stable_diff.baseline_stable_block_count` (`number | null`): baseline bundle의 stable block 수입니다. 유효한 baseline bundle이 없으면 `null`입니다.
- `prompt_bundle_diff.stable_diff.matching_prefix_blocks` (`number`): stable prefix 시작 지점부터 동일하게 유지된 stable block 수입니다.
- `prompt_bundle_diff.stable_diff.stable_prefix_preserved` (`boolean | null`): stable prefix가 완전히 보존되었는지 여부입니다. 유효한 baseline bundle이 없으면 `null`입니다.
- `prompt_bundle_diff.stable_diff.first_stable_invalidation_reason` (`string | null`): stable prefix가 바뀐 경우 첫 stable block 변경 사유입니다.
