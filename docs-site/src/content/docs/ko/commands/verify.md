...
`mf verify`가 실제로 명령을 실행할 때는 plan-only 출력과 같은 예정 실행 모델을 사용하며, 기본 병렬 실행 수는 4입니다. 실제 상한은 로컬 CPU와 최대값 8에 맞춰 줄어듭니다. 효과가 명시되어 있고 서로 충돌하지 않는 같은 예정 실행 묶음의 항목만 동시에 실행할 수 있으며, 효과 정보가 부족한 명령은 보수적으로 처리합니다. 직렬 실행에는 `--parallel 1`을 사용하세요. 영수증은 예정 순서대로 기록되며 verify 출력, 검증 묶음 매니페스트, latest 포인터, 의도별 영수증은 같은 `verification_plan_id`를 공유합니다.

`--profile edit|commit|release`는 작업 단계에 맞춰 검증 비용을 조절합니다. 기본값인 `edit`는 선언된 예상 비용 15초 안에서 충분한 최소 검사를 고르고, `commit`은 60초까지 범위를 넓히며, `release`는 적용 가능한 검사를 모두 유지합니다. 보안, 개인정보, 데이터, 마이그레이션, 패키지 메타데이터, 패키징, 릴리스 이유는 빠른 프로필에서도 전체 검증을 유지합니다. 이전 성공 영수증은 검증 계획과 현재 상태 해시가 같고, 프로필이 `release`가 아니며, 위험도가 높음 미만일 때만 재사용합니다.
