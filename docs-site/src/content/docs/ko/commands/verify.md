...
`mf verify --from-classification <path>`는 mustflow 루트 안의 JSON 파일에서 검증 이유를 읽습니다. 이 파일은 `schema_version: "1"`, `command: "classify"`, 현재 `mustflow_root`, `summary.validationReasons`를 포함한 mustflow 분류 보고서여야 합니다. 그래서 손으로 만든 느슨한 JSON이 실수로 실행 가능한 검증 명령을 고르지 못하게 합니다. `--from-plan`은 이름 전환 기간 동안 같은 동작을 하는 호환 옵션으로 유지됩니다.
...
- 분류 파일은 mustflow 루트 안에 있어야 하며 JSON이어야 하고, 지원되는 `mf classify` 보고서 형식을 따라야 합니다.
