...
반환합니다. 에이전트가 어떤 스킬 문서를 읽을지 고르기 위해 거대한 `.mustflow/skills/INDEX.md`
표 전체를 프롬프트에 넣는 일을 줄이기 위한 명령입니다.
...
- `signals` (`object`): 토큰화된 작업·경로 용어, 이유, resolver가 읽은 route shard입니다.
...
- `source_files` (`string[]`): resolver가 사용한 route 원본 파일입니다.
