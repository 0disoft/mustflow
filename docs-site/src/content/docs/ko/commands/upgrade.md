...
갱신 계획은 관리 파일을 `manifest.lock.toml`과 비교합니다. 사용자 지정 `AGENTS.md`, 명령 계약, 스킬 색인, route 표, 그 밖의 관리 파일은 로컬 변경 차단 항목으로 나타날 수 있습니다. 이는 파일을 지우거나 강제로 덮어쓰라는 뜻이 아니라 멈춰야 한다는 신호입니다.

현재 템플릿과 로컬 계약을 검토하고 필요한 변경만 병합한 뒤, 저장소가 선언한 manifest-lock 절차로 검토된 기준선을 기록하세요. 이후 `mf check --strict`를 실행합니다. `mf upgrade`가 사용자 지정 내용을 자동 병합하지는 않습니다.
