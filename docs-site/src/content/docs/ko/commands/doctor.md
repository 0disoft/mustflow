...
- 실행 가능한 명령 의도가 넓은 호스트 환경 변수를 상속하는지 여부
...
- 엄격 모드에서 `.mustflow/skills/INDEX.md` 경로와 해당 `SKILL.md` 앞부분
  메타데이터가 서로 맞는지 확인했는지 여부
...
- `command_environment` (`object`): 넓은 호스트 환경 변수를 상속하는 실행 가능 명령 의도 요약입니다.
...
- `diagnostics` (`object[]`): 항목별 진단 결과(설치, 검증, 스킬 라우팅, 명령 계약, 읽기 순서 등) 목록입니다.
...
- `check.warning_count` (`number`): 실행을 막지는 않는 검증 경고 수입니다.
- `check.warnings` (`string[]`): 실행을 막지는 않는 검증 경고 메시지 목록입니다.
...
- `command_environment.inherited_intents` (`string[]`): 실제로 `env_policy = "inherit"`를 쓰는 에이전트 실행 가능 명령 의도 목록입니다.
- `command_environment.inherited_network_intents` (`string[]`): 호스트 환경 변수를 상속하면서 `network = true`도 설정한 명령 의도 목록입니다.
...
엄격 모드에서는 `skill_routes` 진단 항목이 `.mustflow/skills/INDEX.md`의
스킬 경로가 실제 파일을 가리키는지, 그리고 표에 적힌 명령 의도가 해당
`SKILL.md` 앞부분 메타데이터에도 선언되어 있는지 요약합니다.
