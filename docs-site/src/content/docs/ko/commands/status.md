...
이 명령은 파일을 수정하지 않습니다. 자동화 검증에는 `mf check`를 쓰고, 사람이 현재 상태를 빠르게 볼 때 `mf status`를 씁니다.
자동화나 에이전트가 읽어야 한다면 `--json`을 사용합니다.

## 보여주는 것

- `Installed`: `AGENTS.md`와 `.mustflow/`가 있는지 나타냅니다.
...
파일이 바뀌었거나 사라졌다면 아래에 해당 경로를 함께 표시합니다.
...
- 종료 코드 `1`: 알 수 없는 선택지를 받은 상태입니다.

변경 파일 때문에 자동화가 실패해야 한다면 `mf status --json`을 읽어 별도로 판단하거나 `mf check`를 사용합니다.
