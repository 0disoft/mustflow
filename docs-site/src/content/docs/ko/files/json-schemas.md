...
- `next-report.schema.json`: `mf next --json` 출력, 선택적 script-pack helper 추천 포함
- `verification-plan.schema.json`: `mf api verification-plan --changed --json` 출력, 선택적
  읽기 전용 script-pack helper 추천 포함
...
- `skill-route-report.schema.json`: `mf skill route --json` 출력. 간결한 route 후보, 선택된
  main·adjunct skill, 점수 분해, route read plan, route 원본 shard, 선택적 읽기 전용
  script-pack helper 추천을 포함합니다.
