...
이 파일은 전체 파일 목록이 아닙니다. `AGENTS.md`, 루트 Markdown 문서, 기계 판독용 계약 파일, `package.json`, `SKILL.md`, `.mustflow/context/INDEX.md`, scaffold 라우터, 검증 가이드, 다이어그램, 언어별 설정 파일처럼 현재 루트를 이해하는 데 도움이 되는 주요 앵커 파일을 찾아, 에이전트가 어디를 먼저 살펴봐야 하는지 알려주는 지도입니다.
...
- 현재 루트가 작업대라면 하위 독립 저장소의 내부 설명이 아니라 진입점만 표시합니다. 하위 저장소가 ssealed식 scaffold 구조를 쓰는 경우에는 scaffold 라우터, 계약 앵커, 최소한의 ssealed manifest 메타데이터를 표시할 수 있습니다.

## 구성요소

- 생성 메타데이터: 문서 정체성, 생성물 여부, 앵커 수, 소스 지문을 안정적인 앞머리 정보로 기록합니다.
- 첫 문장: 전체 파일 목록이 아니라 앵커 파일 기반 탐색 지도라는 점을 밝힙니다.
- 사용 방법: 전체 목록이 필요하면 `git ls-files`를 쓰도록 안내합니다.
- 우선 앵커: `AGENTS.md`, `.mustflow/config/*.toml`, `.mustflow/context/INDEX.md`, `.mustflow/skills/router.toml`, `.mustflow/skills/routes.toml`, `.mustflow/skills/INDEX.md`처럼 먼저 읽거나 라우팅에 필요한 파일을 보여줍니다.
- 디렉터리 앵커: 각 폴더 안의 `README.md`, `AGENTS.md`, `package.json`, `SKILL.md`, `.agents/context-map.md`, 검증 가이드, 다이어그램, 주요 설정 파일을 묶어서 보여줍니다.
...
- `node_modules`, `dist`, `build`, `.git`, `.mustflow/backups`, 캐시, 대용량 산출물은 제외합니다.
- 파일 본문을 요약하지 않습니다.
- 파일 상단에는 다시 생성 여부를 판단하는 데 필요한 안정적인 생성 메타데이터만 둡니다. 생성 시각, 브랜치명, 원격 저장소 주소, 최근 변경 요약, 긴 로그처럼 실행할 때마다 쉽게 달라지는 값은 넣지 않습니다.
- `mf check --strict`는 `source_fingerprint`와 현재 앵커 목록을 비교해, 지도가 오래되었으면 다시 생성하라고 알려줍니다.
...
- `CHECKLIST.md`, `VALIDATION.md`, `.agents/context-map.md`, `.agents/checklists/*.md`, `.agents/validations/*.md`, `.agents/skills/*/SKILL.md`, `.ssealed/manifest.json`, `diagrams/*.mmd`, GitHub 템플릿, API 예시, `db/schema.dbml` 같은 scaffold 및 검증 앵커가 있으면 포함합니다. ssealed manifest는 scope, profile, density, runner, version, 파일 kind 개수처럼 안정적인 필드만 요약하고 checksum이나 생성 시각은 출력하지 않습니다.
