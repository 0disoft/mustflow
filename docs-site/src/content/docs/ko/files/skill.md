...
에이전트는 `.mustflow/skills/router.toml`에서 현재 작업과 맞는 스킬 범주를 고르고, 상세 라우트 메타데이터가 필요할 때 `.mustflow/skills/routes.toml`을 읽으며, 사람이 읽을 라우트 근거가 필요할 때만 `.mustflow/skills/INDEX.md`를 읽은 뒤 선택한 `SKILL.md`를 확인합니다.
