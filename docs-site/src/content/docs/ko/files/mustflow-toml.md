...
- `prompt_cache`: 안정 지시문, 작업 맥락, 변동 상태를 나누는 캐시 친화적 프롬프트 계층입니다.
...
안정 계층은 저장소 규칙과 워크플로 설정처럼 같은 순서로 앞쪽에 둘 수 있는 항목입니다. 작업 계층은 작업마다 선택합니다. 변동 계층은 현재 사용자 요청, 변경 파일 목록, 명령 출력 일부, 실행 기록, 시각, 로컬 경로처럼 자주 바뀌는 값입니다.
...
`default_method = "hash_check"`는 새로고침 지점에서 전문을 다시 읽거나 다시 넣기 전에 내용 해시를 확인할 수 있음을 뜻합니다. `reread_when_hash_changed = true`는 현재 파일을 권위로 유지합니다. `reuse_cached_prefix_when_unchanged = true`는 관련 파일이 바뀌지 않았을 때 호스트가 안정 프롬프트 맥락을 재사용할 수 있게 합니다.
...
`refresh.levels`는 새로고침 수준별로 확인하거나 다시 읽을 파일을 정합니다. 기본 수준은 `light`, `command`, `edit`, `report`, `skill`, `full`입니다. 각 수준은 `method = "hash_check"` 또는 `method = "reread_if_changed"`를 사용할 수 있습니다.
