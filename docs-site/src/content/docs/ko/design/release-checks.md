...
배포 전에는 로컬 소스에서 명령이 되는지만 보지 말고, 실제 npm 포장 파일을 임시 프로젝트에 설치한 뒤 공개 명령이 동작하는지 확인해야 합니다.
...
- `bun run release:check`: CLI 검사, 문서 사이트 검사, 실제 npm 포장과 설치 검증을 한 번에 실행합니다.
- `bun run check:pack`: `npm pack --dry-run --json`으로 패키지에 들어갈 파일 목록을 확인합니다. 이 과정에서 `prepack`이 먼저 빌드합니다.
...
GitHub Pages 설정에서는 `Deploy from a branch`가 아니라 `GitHub Actions`를 배포 원본으로 선택합니다.
...
그 뒤 `docs-site/dist`를 GitHub Pages artifact로 업로드하고 Pages 환경에 배포합니다.
...
이 검사는 패키지 내부의 `dist/`와 `templates/`가 실제 설치 환경에서 함께 동작하는지 확인합니다.
