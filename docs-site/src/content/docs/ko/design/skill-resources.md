...
빈 `references/`, `assets/`, `scripts/` 폴더를 미리 만들지 않습니다. 보조 자원은 스킬 문서가 너무 길어지거나, 반복 실행 보조 도구가 실제로 필요해졌을 때만 추가합니다.
...
`resources.toml`은 보조 자원의 목록과 사용 조건을 기록하는 선택 파일입니다. 스킬 본문을 대체하지 않고, 에이전트가 어떤 자료를 언제 읽거나 실행해도 되는지 판단하는 데 도움을 줍니다.
...
비밀정보, 실행 로그 원본, 생성된 캐시, 대용량 파일은 두지 않습니다.
...
- 입력과 출력 형식이 명확합니다.
- 파일을 쓰거나 네트워크를 사용한다면 `resources.toml`과 `commands.toml`에 드러냅니다.
- destructive 작업은 기본값으로 두지 않습니다.

에이전트는 스크립트 경로를 추측해 직접 실행하지 않습니다. 실행해야 한다면 먼저 `.mustflow/config/commands.toml`에서 연결된 명령 의도를 확인합니다.

## skills/INDEX.md와의 관계

`.mustflow/skills/INDEX.md`는 스킬 단위의 목록만 담습니다. 각 스킬의 모든 보조 파일을 펼쳐 쓰지 않습니다.

보조 파일 목록은 해당 스킬 폴더의 `resources.toml`에서 관리합니다.

## 커뮤니티 스킬 저장소 방향

mustflow core는 기본 스킬을 계속 늘리지 않습니다. 기본 템플릿은 작게 유지하고, 추가 스킬은 나중에 별도 커뮤니티 저장소에서 설치하는 방향을 검토합니다.

가능한 저장소 이름은 `mustflow-skills` 또는 `mustflow-community-skills`처럼 mustflow 규약과 연결되는 이름이 좋습니다. 범위가 너무 넓거나 다른 생태계와 혼동될 수 있는 이름은 피합니다.

커뮤니티 스킬 저장소를 도입한다면 각 스킬은 `SKILL.md`와 mustflow용 `skill.toml`을 함께 가져야 합니다. `skill.toml`은 스킬 식별자, 버전, 호환되는 mustflow 범위, 라이선스, 포함된 스크립트, 네트워크 사용 여부, 파일 쓰기 범위, 위험 등급을 선언합니다.

여러 스킬 묶음은 “자동화 스킬”이 아니라 `pack` 또는 `bundle`로 부릅니다. pack은 스킬을 설치할 뿐, 명령을 자동 실행하거나 `.mustflow/config/commands.toml`을 자동 수정하지 않습니다. 필요한 명령 의도는 권장 항목으로만 보고하고, 실제 선언은 사용자가 프로젝트에 맞게 작성해야 합니다.

나중에 `mf skill add`나 `mf pack add`를 도입한다면 다음 안전장치가 먼저 필요합니다.
...
- 도구별 네이티브 스킬 위치로 내보내는 기능은 기본 설치가 아니라 선택 adapter로만 둡니다.
...
현재 기본 템플릿에는 아직 실제 `resources.toml`을 넣지 않습니다. 먼저 형식과 기준을 문서화하고, 실제 스킬이 복잡해질 때만 도입합니다.
대용량 파일, 비밀정보, 캐시 파일 검사는 보존 정책과 문맥 파일 검사처럼 별도 저장소 안전 검사로 확장할 수 있습니다.
