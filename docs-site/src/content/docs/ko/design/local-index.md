...
SQLite는 이 파일을 더 빠르게 찾고 분석하기 위한 보조 색인입니다. 지워도 다시 만들 수 있어야 합니다.
...
이 값은 색인이 기준 원본이 아니라, 필요할 때 다시 만들 수 있는 선택형 생성물이라는 뜻입니다.
...
현재 `mf index`는 mustflow 문서, 맥락 파일, 설정 파일, 스킬 문서, 명령 의도를 색인합니다.
...
원본 로그, 터미널 출력 전체, 대화 원문 전체는 색인이나 지식 문서의 기준 원본이 아닙니다.
