...
`indexed_files` 테이블은 색인된 워크플로 파일과 선택적으로 포함된 소스 앵커 파일의 파생 지문을 저장합니다. 저장 항목은 경로, 출처 범위, 크기, 수정 시각, 내용 해시, 색인 시각, 색인 모드, 파서 버전입니다. `mf index --incremental`은 스키마, 파서 버전, 소스 범위 설정, 파일 지문이 모두 호환될 때만 기존 SQLite 파일을 재사용하고, 그렇지 않으면 전체 재생성으로 돌아갑니다.
