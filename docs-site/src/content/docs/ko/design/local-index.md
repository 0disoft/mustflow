...
검색 메타데이터는 `search_ngrams` 테이블에도 저장됩니다. 이 행들은 짧은 파생 용어 조각이며, 공백이나 SQLite 토큰화가 약한 다국어 검색을 보조합니다. 문서, 스킬, 스킬 라우트, 명령 의도, 소스 앵커를 가리킬 뿐 전체 문서나 소스 본문을 저장하지 않고, 권한 정렬도 바꾸지 않습니다.
