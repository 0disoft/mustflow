...
이 파일에는 저장소별 작업 규칙이 담겨 있기 때문에 자동 갱신은 신중해야 합니다.
...
잠금 항목의 `last_action`이 `customized`라면, mustflow는 그 해시를 저장소가 의도적으로 바꾼 기준선으로 봅니다. 현재 파일 해시가 그 맞춤 기준선과 여전히 같으면, 새 템플릿과 달라도 템플릿 내용으로 바꾸지 않습니다.
...
실제 파일을 바꾸는 `mf update --apply`는 아래 규칙을 따릅니다.
...
백업은 사용자가 수정한 파일을 보호하기 위한 마지막 안전장치입니다. 단, 백업이 있다고 해서 `blocked-local-change`를 자동 덮어써도 된다는 뜻은 아닙니다.
...
- `mf update`만 실행하고 `--dry-run`, `--apply`를 고르지 않으면 종료 코드 `1`입니다.
