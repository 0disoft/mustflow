...
| Git이 CRLF/LF 경고를 내거나 추적 중인 텍스트 파일에 줄바꿈 정규화가 필요할 수 있음 | `.mustflow/skills/line-ending-hygiene/SKILL.md` | 경고 문구 또는 변경 파일 근거, 줄바꿈 정책, 변경 파일 상태, 명령 계약 항목 | 줄바꿈 정책 파일, 추적 중인 텍스트 파일, 명령 메타데이터, 테스트, 보고 | 조용한 작업 트리 재작성 또는 정책 불일치 | `line_endings_check`, `changes_status`, `mustflow_check` | 확인한 정책, 줄바꿈 불일치 파일, 정규화 여부, 검증, 남은 줄바꿈 위험 |
