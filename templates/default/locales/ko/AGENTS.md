...
10. 라우트 메타데이터만으로 판단이 부족하거나 스킬 라우팅을 수정하거나 상세 라우트 표가 필요한 경우에만 `.mustflow/skills/INDEX.md`
11. 현재 작업과 관련된 `.mustflow/skills/<name>/SKILL.md`
12. 넓은 저장소 탐색이 필요할 때만 `REPO_MAP.md`
13. 관련 소스, 테스트, 문서 파일
...
- 파일을 만들거나 수정하기 전에 `.mustflow/skills/routes.toml`을 기준으로 현재 작업에 적용 가능한 스킬이 하나 이상 있는지 판단합니다. 압축된 라우트 메타데이터만으로 부족하거나, 스킬 라우팅을 수정하거나, 상세 라우트 표가 필요할 때만 `.mustflow/skills/INDEX.md`를 읽습니다. 이 스킬 선택 단계는 작업이 작거나 명백해 보이는 경우에도 반드시 거쳐야 합니다.
