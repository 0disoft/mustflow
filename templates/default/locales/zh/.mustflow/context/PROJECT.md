...
- 本文件可以记录有依据的上下文、未知事项和冲突。
- 本文件不得授予命令执行权限、定义文件编辑禁令、覆盖 `AGENTS.md` 或
  `.mustflow/config/*.toml`，也不得承诺当前来源没有支撑的功能。
- 持久的操作规则应放在 `AGENTS.md`、`.mustflow/docs/agent-workflow.md` 或对应
  的配置文件中，而不是存放在这里。
