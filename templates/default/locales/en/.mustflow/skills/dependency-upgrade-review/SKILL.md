...
8. For security upgrades, keep the patch narrow. Identify advisory id, affected range, fixed range, direct or transitive path, exploit-relevant code path, minimum patched version, scanner recheck, and whether stricter validation, escaping, TLS, redirect, auth, or parser behavior can break callers.
   - Build one root-cause ledger keyed by advisory identity, ecosystem, package, resolved version, and manifest. Treat reports from Dependabot, OSV, CodeQL, Scorecard, or another detector as corroborating signals when they identify the same vulnerable graph or workflow reference; do not inflate duplicate detector records into separate vulnerabilities or discard the extra detector coverage.
   ...
   - After regenerating the lockfile, confirm the old vulnerable version is absent from the resolved graph and that any override is recorded in the manifest rather than hidden as unexplained lockfile churn. Treat local graph proof and remote alert closure as separate evidence: require a same-ref scanner rerun or current registry alert state before claiming the remote alert is closed.
...
17. For Docker base images and CI actions, review them as dependencies. Check image/action source, version pinning policy, digest use when required, runtime version changes, security patch reason, cache impact, and deployment smoke coverage. Pin third-party GitHub Actions to the verified full commit SHA, retain the reviewed release or channel as a comment, and record tag-to-commit evidence; a mutable rolling tag or branch is not an immutable dependency merely because the owner is trusted.
